...
const CleaverTestPage = () => {
  const { token } = useParams<{ token: string }>();
  const navigate = useNavigate();
  const { getTestByToken, startTest } = useCleaver();
  ...
  const handleStartTest = async () => {
    ...
    try {
      ...
      navigate(`/cleaver-test/${token}/start`);
    } catch (err) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-green-500/20 border border-green-500/30 rounded-xl p-8 max-w-md text-center">
          ...
          <p className="text-green-300 mb-6">
            ¡Gracias! Ya has completado este test psicométrico.
          </p>
          <p className="text-white/70 text-sm">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-8 max-w-lg w-full">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Test Psicométrico Cleaver</h1>
          <p className="text-white/70">Evaluación de personalidad laboral</p>
        </div>
        ...
        <div className="bg-blue-500/20 border border-blue-500/30 rounded-lg p-4 mb-6">
          ...
          <ul className="text-blue-200 text-sm space-y-2">
            <li className="flex items-start space-x-2">
              ...
              <span>El test consta de 24 grupos de palabras</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-blue-400 mt-1">•</span>
              <span>En cada grupo, selecciona la palabra que MÁS te describe y la que MENOS te describe</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-blue-400 mt-1">•</span>
              <span>Duración aproximada: 15-20 minutos</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-blue-400 mt-1">•</span>
        ...
        <div className="text-center">
          {!hasStarted ? (
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={() => navigate(`/cleaver-test/${token}/start`)}
        ...
        <div className="mt-8 text-center">
          <p className="text-white/50 text-xs">
            Test Cleaver DISC - Evaluación de Personalidad Laboral
          </p>
        </div>
      </div>
    </div>
...
export default CleaverTestPage; 